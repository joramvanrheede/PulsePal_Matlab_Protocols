...
%% do st for this
whisk_sustain           = [.05]; % how long (in seconds) to sustain the whisker deflection. 
                        ...
                        for g = 1:length(amplitudes) % for all whisking amplitudes
                            for h = 1:length(LED_powers) % for all LED powers
                                for i = 1:length(LED_trig_freqs)
                                    ...
                                    block_mat(counter,:)    = [whisk_delays(a) whisk_wave_freq(b) whisk_trig_freq(c) LED_delays(d) LED_durations(e) f amplitudes(g) LED_powers(h) LED_trig_freqs(i)]; % ...and generate a trial, appending it to the block.
                                end
                            end
                        end
                    end
